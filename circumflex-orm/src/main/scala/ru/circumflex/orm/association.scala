...
class Association[K, C <: Record[_, C], P <: Record[K, P]] (
  ...
  override def value()(implicit ormConf: ORMConfiguration): Option[P] =
    field.value.flatMap(id => parentRelation.get(id))

  override def set(v: Option[P])(implicit ormConf: ORMConfiguration): this.type = {
  ...
  def IS(record: P)(implicit ormConf: ORMConfiguration): Predicate =
    new SimpleExpression(ormConf.dialect.EQ(aliasedName, placeholder), List(record.PRIMARY_KEY.get))
  def IS_NOT(record: P)(implicit ormConf: ORMConfiguration): Predicate =
    new SimpleExpression(ormConf.dialect.NE(aliasedName, placeholder), List(record.PRIMARY_KEY.get))
  def IN(params: Iterable[P])(implicit ormConf: ORMConfiguration): Predicate = new SimpleExpression(
    ormConf.dialect.parameterizedIn(aliasedName, params.map(p => placeholder)),
    params.map(_.PRIMARY_KEY.get).toList)

  def joinPredicate(childAlias: String, parentAlias: String)(implicit ormConf: ORMConfiguration): Predicate = {
